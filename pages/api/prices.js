...
export default async function handler(req, res) {
  ...
  const lastPublishmentDate = Object.keys(_.groupBy(data, (i) => i.date)).sort(
    (a, b) => new Date(b) - new Date()
  )[0];
  ...
  const prices = _.reject(lastPublishedDataset, {
    ...
    adjusted_price_index:
      (parseFloat(item.dollar_ex) * parseFloat(item.adj_price)) /
      (parseFloat(source.dollar_ex) * parseFloat(source.adj_price)),
  }));
